function [time,outputS,outputR,Nt,break_time,sum_Energy_ions,time_fir_R,n_fir_R,time_fir_S,n_fir_S] = trial_R(gPoisson,NS_node,NR_node,step,gAMPA_SS,gNMDA_SS,gGABA_SS,gRtoS,gAMPA_RR,gNMDA_RR,gGABA_RR,gStoR...
    ,matrixS,matrixR,matrixRtoS,matrixStoR)

addpath 'fun\' 'tool\' 'connect\' 'func_analysis\' 'main_fun\'

%% Par
T_tot=6000; N_print=1;
Nt=round(T_tot/step);
...
for iii  = 1:Nt
    ...
    IextR = 0;
    if time(iii)>5500 && time(iii)<5800
        IextR=I0_R;
%         IextR=2;
    end
